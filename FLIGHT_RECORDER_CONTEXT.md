...
Public-domain deployment is retired from the iPad App runtime plan. The iPad App is local-only. App Store support/privacy pages use GitHub Pages, but only as public static pages and not as an app backend/runtime dependency. GitHub Pages is enabled from `main` / `docs` and was verified live on 2026-06-22. Target App Store regions: Mainland China and United States. The app runtime is offline, does not provide an internet information service, and should be described as having no ICP/app filing requirement for the app runtime.
...
- Capacitor 8.4.0 iOS project exists under `ios/App`; Bundle ID is `com.xiazhiyuan.flightrecorder`, version `1.0`, build `3`, iPad-only, minimum iPadOS 15.0.
...
- A Release signed archive preflight also succeeded at `/private/tmp/FlightRecorder-v1.0-b3.xcarchive` using `npm run ios:archive:signed:tmp`; `node scripts/verify_xcode_archive.mjs --require-signed /private/tmp/FlightRecorder-v1.0-b3.xcarchive` confirmed Bundle ID `com.xiazhiyuan.flightrecorder`, version `1.0`, build `3`, and signing metadata. Treat this as a preflight artifact; final upload should still go through Xcode Organizer Validate/Distribute.
- App Store Connect App record exists; Xcode found App ID `6782894756`. Build `1.0 (3)` upload/export succeeded from signed archive `/private/tmp/FlightRecorder-v1.0-b3.xcarchive`; upload succeeded on 2026-06-22 21:53 CST with the existing AppIcon explicitly declared. Delivery UUID: `cb751285-1d4b-438b-87ff-5c49572b3c71`. Apple reported the uploaded package is processing, so the next step is to wait for processing to finish and select build `1.0 (3)` on the App Store Connect version page.
